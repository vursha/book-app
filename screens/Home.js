    ...
    <View style={styles.container}>
      ...
      <View style={styles.box3}>
        <View style={styles.view3}>
          ...
          <View style={{flexDirection:"row"}}>
          <Image 
            style={{width:20,height:20,marginTop:20,marginLeft: 2}}
            source={point_icon}
            />
      ...
      <View style={{marginLeft:30,flexDirection:"row",justifyContent:"space-between",marginTop:10}}>
        {Titles.map((obj,index)=> (
        <TouchableOpacity 
        key={index}
        onPress={()=> setSelectedItem(obj.id)}>
          {
            selectedItem === obj.id && 
            <Text style={{color:"white",fontSize:20,fontWeight:"bold"}}>{obj.name}</Text>
          }
           {
            selectedItem !== obj.id && 
            <Text style={{color:"grey",fontSize:20,fontWeight:"bold"}}>{obj.name}</Text>
          }
          {/* <Button></Button> */}
